...
Provides traces (ddtrace), metrics (DogStatsD), and trace-correlated logs, and
bridges Lance's own structured trace events into Datadog. Telemetry clients are
created per process via :meth:`Telemetry.create`, so the lightweight
:class:`TelemetryConfig` is the only object pickled into executor closures.

Lance emits structured trace events (file audits, dataset events, object-store
throttling, index I/O, and execution stats) through a non-blocking callback that
:func:`attach_lance_event_bridge` registers; the bridge turns those events into
Datadog counters, gauges, and distributions and forwards them as logs. The
bridge attaches automatically the first time :meth:`Telemetry.create` runs in a
process, so driver and executors both report Lance internals. The performance
tuning that the Lance guide describes (compute and I/O thread pools, the scan
read-ahead buffer, log and trace levels) is applied through
:class:`LanceRuntimeConfig` and :func:`apply_lance_runtime`.

Assumes a Datadog Agent reachable from every node for DogStatsD on the
configured host and port. The ddtrace, datadog, and lance imports use version
and availability fallbacks; adjust the import block if installed versions
differ.
"""
...
try:
    from lance import CommitConflictError
except ImportError:
    try:
        ...
        class CommitConflictError(Exception):
            """Fallback used when pylance is unavailable at import time."""
...
class TelemetryConfig:
    ...
    Attributes:
        ...
        constant_tags: Tags attached to every metric.
    """
...
class LanceRuntimeConfig:
    ...
    The thread-pool and buffer settings must be in the environment before Lance
    initializes its pools, so on Spark they belong in the executor environment
    rather than being set inside a task. :func:`apply_lance_runtime` sets them on
    the current process for the driver, and returns the variables it set so the
    caller can mirror them onto executors.

    Attributes:
        cpu_threads: Value for ``LANCE_CPU_THREADS``; set this below the executor
            core count when several tasks share an executor to avoid compute
            oversubscription.
        io_threads: Value for ``LANCE_IO_THREADS``; cloud stores often need 128
            or 256 to saturate bandwidth.
        io_buffer_size_bytes: Value for ``LANCE_DEFAULT_IO_BUFFER_SIZE``; raise it
            alongside the I/O thread count.
        lance_log: Value for ``LANCE_LOG`` controlling log filtering by level and
            target.
        lance_tracing: Value for ``LANCE_TRACING`` controlling the trace level
            the event bridge observes; the key events are emitted at ``info``.
    """

    cpu_threads: Optional[int] = None
    io_threads: Optional[int] = None
    io_buffer_size_bytes: Optional[int] = None
    lance_log: Optional[str] = None
    lance_tracing: Optional[str] = None
...
def commit_with_retries(
    ...
    The action should re-read any dataset state it needs so each retry observes
    the latest committed version. Backoff doubles per attempt up to a cap.
    ...
    Raises:
        RuntimeError: If every attempt conflicts.
    """
    ...
    raise RuntimeError(f"commit failed after {retries} conflict retries")
...
def attach_lance_event_bridge(telemetry: "Telemetry") -> bool:
    ...
    Subsequent calls within the same process are no-ops, and the call is a no-op
    when the installed pylance does not expose the trace-event API.
...
class Telemetry:
    ...
    def create(cls, config: TelemetryConfig, attach_lance_bridge: bool = True) -> "Telemetry":
        ...
        Call this on the driver and again inside each executor partition so the
        clients are local and never pickled. The Lance trace-event bridge is
        attached on the first call per process unless disabled.
    ...
    @contextmanager
    def span(
        self, name: str, resource: Optional[str] = None, tags: Optional[Dict[str, object]] = None
    ) -> Iterator[object]:
    ...
    def incr(self, name: str, value: float = 1, tags: Optional[List[str]] = None) -> None:
    ...
    def distribution(self, name: str, value: float, tags: Optional[List[str]] = None) -> None:
    ...
    def event(self, name: str, tags: Optional[List[str]] = None) -> None:
    ...
    def error(self, message: str, tags: Optional[List[str]] = None) -> None:
        """Log the current exception and increment an error counter.

        Call this from an ``except`` block; the active exception and traceback
        are logged before the caller re-raises to fail fast.
    ...
    @contextmanager
    def timed(self, name: str, tags: Optional[List[str]] = None) -> Iterator[None]:
    ...
    def current_context_ids(self) -> Optional[Tuple[int, int]]:
