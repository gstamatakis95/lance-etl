...
Supports the common scalar types plus ``timestamp``, ``list<...>``, and
``fixed_size_list<..., size>`` so cast targets can be supplied on the command
line. Richer or deeply nested targets should be built as ``pyarrow`` types and
passed to the job programmatically.
"""
...
SCALAR_TYPES: Dict[str, Callable[[], pa.DataType]] = {
    "bool": pa.bool_,
    "boolean": pa.bool_,
    "int8": pa.int8,
    "int16": pa.int16,
...
def resolve_arrow_type(spec: str) -> pa.DataType:
    ...
    Args:
        spec: A type spec such as ``"float16"``, ``"timestamp[us, UTC]"``,
            ``"list<float16>"``, or ``"fixed_size_list<float16, 768>"``.
    ...
    if lowered.startswith("timestamp"):
        ...
        parts: list = [p.strip() for p in inside.split(",")] if inside else ["us"]
