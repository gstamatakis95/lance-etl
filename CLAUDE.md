...
2. Lance indexes are built exclusively via the segment API (`create_index_uncommitted` /
   ...
   k-means and an explicit stored `rabitq_model` (ADR 0030). ZONEMAP is a segment-API scalar type
   too, but unlike BTREE/BITMAP its segments are merged with `merge_existing_index_segments`
   before commit (ADR 0033). Never use `create_scalar_index(fragment_ids=)` for BTREE, BITMAP, or
   ZONEMAP — it raises on current lance main.
