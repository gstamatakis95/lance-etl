"""Per-dataset maintenance for a fleet of per-tenant Lance datasets: TTL expiration, compaction, and version cleanup.

A maintenance run applies three ordered steps to each dataset in one consolidated executor pass. First, when a per-row
TTL column is configured, expired rows are deleted. Second, the dataset is classified and compacted with the two-tier
orchestration below. Third, old versions are pruned. All steps share the same initial dataset open, so the fleet
traversal makes exactly one ``lance.dataset()`` call per dataset instead of multiple separate passes, reducing S3 LIST
round-trips on large fleets. A single :func:`maintain_one_dataset` executor function wraps the TTL delete and
classify-or-compact logic with failure isolation: a corrupt or missing dataset returns a skip dict and never aborts
the fleet run.
...
Delete-predicate safety: both column names are validated against the dataset schema before any predicate is
constructed, and the cutoff instant is formatted internally as a typed ``TIMESTAMP`` literal.
...
class MaintenanceConfig:
    ...
    Attributes:
        ...
        ttl_column: Per-row TTL column holding each row's lifetime as an Arrow ``Duration``; ``None`` (default) turns
            TTL off. When set, rows where ``ts_column + ttl_column < now`` are deleted before compaction.
        ts_column: Event timestamp column used as the TTL clock; must match ``ETLConfig.ts_col``.
        target_rows_per_fragment: Desired rows per compacted fragment; matches lance's ``CompactionOptions`` default
            of ``1_048_576`` so the default is explicit and immune to upstream shifts.
        max_rows_per_group: Maximum rows per group within a fragment.
        max_bytes_per_file: Maximum bytes per compacted file.
        materialize_deletions_threshold: Deletion fraction above which a fragment is rewritten; default ``0.5``
            defers materialisation until half the rows are gone to avoid triggering the IVF_RQ full-rebuild guard on
            every maintenance cycle for indexed fragments.
        defer_index_remap: Defer index remap on the small-dataset tier only; the large-dataset tier ignores it because
            the Python ``Compaction.commit`` binding always remaps indices inline.
        max_source_fragments: Cap on source fragments consumed per run for incremental compaction; ``None`` is
            unbounded, ``0`` is rejected.
        ...
        large_dataset_fragment_threshold: Fragment count above which a dataset uses the distributed plan/execute/commit
            path instead of in-process ``Compaction.execute``.
        ...
        cleanup_older_than_seconds: Age threshold for version cleanup; default ``172_800`` (2 days) with the HEAD-tag
            exemption keeps rollback headroom while cutting manifest storage. ``None`` defers to lance's 14-day
            default. Values below ``min_cleanup_horizon_seconds`` are rejected.
        ...
        large_commit_retries: Retry budget around tier-B ``Compaction.commit``; kept small because semantic conflicts
            re-fail deterministically and only the raw manifest-write race benefits from a retry.
        replan_budget: Plan/execute/commit cycles per tier-B dataset before skipping as hot.
        compaction_mode: Lance compaction mode for ``Compaction.execute`` and ``Compaction.plan``; ``try_binary_copy``
            falls back to reencode per task, never errors on deletion-bearing fragments unlike ``force_binary_copy``.
        min_cleanup_horizon_seconds: Floor for ``cleanup_older_than_seconds``; cleanup is not transactional, so the
            floor must exceed the longest concurrent job to protect in-flight committer rebase files.
    """
    ...
    materialize_deletions_threshold: float = 0.5
    ...
    def execute_options(self) -> dict[str, Any]:
        ...
        Deleted rows are always materialized and the mode is taken from :attr:`compaction_mode`.

        Returns:
            Options accepted by ``Compaction.execute``, omitting unset optional values.
    ...
    def plan_options(self) -> dict[str, Any]:
        ...
        Returns:
            Options accepted by ``Compaction.plan``, omitting unset optional values.
        """
...
def validate_column_name(column: str, schema: Any) -> None:
    ...
    Catches silent misconfigurations where the column name was changed but the config was not updated, before any
...
def build_ttl_predicate(ts_column: str, ttl_column: str, cutoff: datetime) -> str:
    ...
    column arithmetic natively. Both column names have already been validated against the dataset schema by
    :func:`validate_column_name` before this function is called.

    The cutoff is formatted in UTC with microsecond resolution as ``YYYY-MM-DDTHH:MM:SS.ffffff``, which DataFusion
    accepts as a timestamp literal.
...
def delete_expired_rows(uri: str, config: MaintenanceConfig, cutoff: datetime, telemetry: Telemetry) -> dict[str, Any]:
    ...
    Validates the event timestamp and TTL column names against the dataset schema, builds the per-row delete
    predicate, and issues the delete through :func:`lance_etl.telemetry.commit_with_retries`. A
...
def run_ttl_on_open_dataset(
    ...
    Validates the TTL and timestamp column names against the schema of the supplied open handle, then delegates the
    actual delete to a re-opening retry action so each commit attempt operates against the latest version (required for
    rebase correctness). A dataset that lacks the TTL or timestamp column is skipped with a warning and a metric rather
...
def fan_out_per_dataset(
    ...
    by the consolidated maintenance pass, the manifest migration, and the serving-tag flip, all of which are
...
def cleanup_dataset(uri: str, config: MaintenanceConfig, telemetry: Telemetry) -> int:
    ...
    Raises:
        ValueError: If ``cleanup_older_than_seconds`` is set below ``config.min_cleanup_horizon_seconds``. The horizon
...
def maintain_one_dataset(
    ...
    Opens the dataset once and runs the TTL delete and classify-or-compact steps in sequence, sharing the open handle
    where possible. A single ``try/except`` around the open call provides failure isolation: a missing, corrupt, or
    unreadable dataset returns a skip dict and never aborts the fleet run.

    Steps executed (each conditional on its guard):

    - TTL delete: when ``config.ttl_active()`` is ``True`` and ``cutoff`` is not ``None``, runs
      :func:`run_ttl_on_open_dataset` for schema validation then issues the delete via a re-opening retry action.
    - Classify and compact: reads the fragment count from the open handle's stats to gate the large-tier fast path,
      then delegates to :func:`classify_or_compact` so that callers patching that module-level function in tests
      continue to intercept the compaction step.
    ...
    Returns:
        A result dictionary merging TTL and compaction fields. Always contains ``uri``, ``tier``, and
        ``bytes_removed``. A skipped dataset has ``tier="skipped"`` and ``skipped`` holding the error message.
        A dataset with TTL active includes ``ttl_rows_deleted``.
    """
    try:
        dataset: lance.LanceDataset = lance.dataset(uri, storage_options=config.storage_options)
...
class MaintenanceJob:
    ...
    def run(self, spark: SparkSession, dataset_uris: Iterable[str]) -> list[dict[str, Any]]:
        ...
        The run is ordered: TTL expiration (when configured) runs first inside the per-dataset executor task, followed
        by two-tier compaction, followed by version cleanup. A single :func:`maintain_one_dataset` executor function
        opens each dataset once and runs the TTL delete and classify-or-compact steps in sequence, sharing the open
        handle. The TTL cutoff is computed once on the driver before the fan-out and shared across all executors so
        every dataset decides expiry against the same clock. Large-tier datasets returned by the fan-out are then
        compacted with the distributed plan/execute/commit triad, several at a time from a driver thread pool on the
        FAIR scheduler pool.

        Run-level aggregates extracted from the outcome dicts include TTL rows deleted and datasets expired, bytes and
        fragments reclaimed, and the standard dataset/tier counts.
