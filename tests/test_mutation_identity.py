...
def test_event_digest_is_stable_across_mapping_order() -> None:
    ...
    first = mutation({"text": "hello", "metadata": {"b": "2", "a": "1"}})
    second = mutation({"metadata": {"a": "1", "b": "2"}, "text": "hello"})
    first_digest = canonical_event_digest(first.target, first.vector_id, "upsert", first.event_timestamp, first.payload)
    second_digest = canonical_event_digest(
...
def test_exact_duplicates_collapse_across_one_hundred_retries() -> None:
    ...
    rows = [mutation({"text": "same"}) for attempt in range(100)]
    terminal = collapse_snapshot_mutations(rows, window_seq=7, source_sequence=91)
    assert len(terminal) == 1
    assert terminal[0].source_sequence == 91
    expected = terminal_source_digest(terminal)
    for replay_rows in [rows] * 100:
        replay = collapse_snapshot_mutations(replay_rows, window_seq=7, source_sequence=91)
...
def test_later_snapshot_source_sequence_is_delivery_order() -> None:
    ...
    first = collapse_snapshot_mutations([mutation({"text": "first"})], 1, 10)[0]
    older_time = MutationInput(
    ...
    later = collapse_snapshot_mutations([older_time], 2, 11)[0]
...
def test_complete_post_image_nulls_omissions_and_rejects_unknown_fields() -> None:
    ...
    with pytest.raises(ValueError, match="outside the release profile"):
...
def test_source_digest_is_partition_and_input_order_independent() -> None:
    ...
    first = b"a" * 32
    second = b"b" * 32
    forward = canonical_source_digest([("z", 2, second), ("a", 2, first)])
    reverse = canonical_source_digest([("a", 2, first), ("z", 2, second)])
...
def test_source_digest_rejects_mixed_targets() -> None:
    ...
    first = collapse_snapshot_mutations([mutation({"text": "first"})], 1, 1)[0]
    other = MutationInput(
        tenant_id="other",
        namespace="namespace",
        org_id="org",
    ...
    second = collapse_snapshot_mutations([other], 1, 1)[0]
